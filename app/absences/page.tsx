...
import { ClipboardList, Plus, ArrowLeft, ArrowRight, Sun, Moon, Trash2, Languages } from "lucide-react";
    ...
    <div dir={dir} style={{ background: th.pageBg, minHeight: "100vh", fontFamily: "'Cairo', sans-serif", transition: "all 0.4s" }} className="p-4 md:p-8">
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ ...th.card, padding: "16px 20px", marginBottom: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
              <div style={{ width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: darkMode ? "rgba(248,113,113,0.12)" : "#fff3f3", border: darkMode ? "1px solid rgba(248,113,113,0.2)" : "none", boxShadow: darkMode ? "0 0 12px rgba(248,113,113,0.3)" : "0 2px 6px rgba(0,0,0,0.06)" }}>
                <ClipboardList size={18} color={th.accent} />
              </div>
              <h1 style={{ fontSize: "18px", fontWeight: "800", color: th.title, textShadow: th.titleShadow, whiteSpace: "nowrap" }}>{t("absences")}</h1>
            </div>
        ...
        <div style={{ ...th.card, padding: "24px", marginBottom: "24px" }}>
          <h2 style={{ fontWeight: "700", color: th.title, marginBottom: "16px", fontSize: "15px", textShadow: th.titleShadow }}>{t("newAbsence")}</h2>
          ...
          <button onClick={addAbsence} onMouseDown={() => setPressed(true)} onMouseUp={() => setPressed(false)} onMouseLeave={() => setPressed(false)}
            style={{ background: darkMode ? "rgba(248,113,113,0.12)" : "rgba(224,92,92,0.1)", border: darkMode ? "1px solid rgba(248,113,113,0.35)" : "1px solid rgba(224,92,92,0.35)", borderRadius: "14px", padding: "11px 22px", color: th.accent, fontWeight: "700", fontSize: "14px", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", fontFamily: "'Cairo', sans-serif", transition: "all 0.2s", transform: pressed ? "scale(0.98)" : "scale(1)" }}>
            <Plus size={17} /> {t("register")}
          </button>
        </div>
        ...
        <div style={{ ...th.card, padding: "24px" }}>
          <h2 style={{ fontWeight: "700", color: th.title, marginBottom: "16px", fontSize: "15px", textShadow: th.titleShadow }}>
            {t("absencesList")} ({absences.length})
          </h2>
          {absences.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px", color: th.subtext }}>
              <div style={{ width: "56px", height: "56px", margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: darkMode ? "rgba(248,113,113,0.08)" : "#fff3f3" }}>
                <ClipboardList size={26} color={th.subtext} />
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {absences.map((a) => (
                <div key={a.id}
                  style={{ ...th.row, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "all 0.2s", transform: hoveredRow === a.id ? "scale(1.01)" : "scale(1)", boxShadow: hoveredRow === a.id ? (darkMode ? "0 4px 20px rgba(0,0,0,0.3)" : "0 4px 12px rgba(0,0,0,0.08)") : "none" }}
                  onMouseEnter={() => setHoveredRow(a.id)} onMouseLeave={() => setHoveredRow(null)}>
                  <button onClick={() => deleteAbsence(a.id)} style={{ background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "8px", width: "32px", height: "32px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                    <Trash2 size={14} color="#f87171" />
                  </button>
                  <div style={{ display: "flex", alignItems: "center", gap: "24px" }}>
                    <p style={{ fontSize: "13px", color: th.subtext, fontWeight: "500" }}>{a.date}</p>
                    <p style={{ fontSize: "14px", color: th.title, fontWeight: "700", textShadow: darkMode ? th.titleShadow : "none" }}>{getStudentName(a.student_id)}</p>
                  </div>
                </div>
              ))}
            </div>
